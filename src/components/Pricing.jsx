...
const Pricing = ({ t }) => {
  return (
    <section id="pricing" className="py-14 sm:py-20 lg:py-32 bg-[#CDE4F1]/50 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="text-center max-w-2xl mx-auto mb-10 sm:mb-14">
          ...
          <p className="mt-3 sm:mt-4 text-base sm:text-lg text-[#5C6173]">{t.pricing.subtitle}</p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-5 md:gap-6 lg:gap-8 mb-12 sm:mb-16">
          {t.pricing.cards.map((c, i) => (
            ...
            >
              <div className={`inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full mb-5 sm:mb-6 ${
                i === 1 ? "bg-[#F4B8C1] text-[#2D3142]" : "bg-[#FDF1BA] text-[#2D3142]"
              }`}>
                <Heart className="w-5 h-5 sm:w-6 sm:h-6" />
              </div>
              <p className={`font-display text-xl sm:text-2xl ${i === 1 ? "text-[#FAD4D8]" : "text-[#5C6173]"}`}>
                {c.time}
              </p>
              <p className="font-display text-6xl sm:text-7xl lg:text-8xl font-bold leading-none mt-2 sm:mt-3">
                {c.price}
              </p>
              <div className={`mt-5 sm:mt-6 flex items-center gap-2 ${i === 1 ? "text-[#CDE4F1]" : "text-[#5C6173]"}`}>
          ...
          <ul className="space-y-3">
            {t.pricing.rules.map((r, i) => (
              <li key={i} className="flex items-start gap-3">
                <span className="mt-1 inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#A8D0E6] flex-shrink-0">
